...
export default function WebhookInput({ id, className="" }: WebhookInputProps) {
  ...
  return (
    <div className={ cn("flex rounded-md flex-col border h-56 p-4 gap-y-4", className) }>
      <form onSubmit={ e => {
        e.preventDefault();
        addWebhook(webhookInput);
      } } className={ "flex w-full h-1/4 gap-x-2" }>
        <Input autoComplete={ "off" } value={ webhookInput } onChange={ e => setWebhookInput(e.target.value) }
               id={id} className={ "h-full w-full" }/>
      ...
      <div className={ "flex flex-col w-full overflow-y-auto rounded-md border h-full p-4" }>
        { webhooks.map((webhook, index) => (
          <p onClick={ () => deleteWebhook(webhook) }
             className={ "text-sm hover:line-through hover:cursor-pointer hover:text-red-600" }
             key={ index }>{ webhook }</p>
        )) }
      </div>
    </div>
